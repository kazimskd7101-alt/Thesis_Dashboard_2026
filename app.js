...
function patchStaticCopy() {
  document.querySelectorAll(".metric-meta span, .queue-panel .panel-head p").forEach(el => {
    const t = (el.textContent || "").trim();
    if (t === "Surfaced cases") el.textContent = "Dashboard rows";
    if (t === "Ranked surfaced cases for analyst review.") el.textContent = "Ranked dashboard cases for analyst review.";
  });
}
...
function buildRows() {
  ...
  state.rows = (state.data.dashboard || []).map((r, idx) => {
    ...
    const labelUp = number(
      getAny(r, ["label_20", "label", "label_up", "y_true"]) ||
      getAny(grad, ["label_20", "label", "label_up", "y_true"]) ||
      getAny(raw, ["label_20", "label", "label_up", "y_true"])
    );
    const inferredPred = inferPredictedClass(pred, pUp, signal);
    const rawCorrect = parseTruth(getAny(r, ["is_correct", "correct"]));
    const correct = rawCorrect !== null ? rawCorrect : inferCorrect(inferredPred, labelUp);
...
function applyFilters() {
  ...
  setText("queue-summary-pill", `${state.filtered.length} dashboard row${state.filtered.length === 1 ? "" : "s"}`);
  renderCharts();
}
...
function renderStudio() {
  ...
  if (!r) {
    ...
    setText("selected-subtitle", "Choose a dashboard review case from the signal queue.");
  ...
  setText("evidence-error", prettyErrorType(r.errorType));

  const hasXai = hasXaiAllocation(r);
  setText("evidence-fg", hasXai ? formatMetric(r.fg, 3, "—") : "N/A");
  setText("evidence-bg", hasXai ? formatMetric(r.bg, 3, "—") : "N/A");

  setBar("fg-bar", hasXai ? Math.max(0, Math.min(100, r.fg * 100)) : 0);
  setBar("bg-bar", hasXai ? Math.max(0, Math.min(100, r.bg * 100)) : 0);
  ...
  setText("decision-note", buildDecisionNote(r, action));
...
function buildDecisionNote(row, action) {
  if (!row) return "No case selected.";

  const predClass = inferPredictedClass(row.pred, row.pUp, row.signal);
  const predictedDirection =
    predClass === 1 ? "up / long" :
    predClass === 0 ? "down / short" :
    prettySignal(row.signal).toLowerCase();
  ...
  const displayError = prettyErrorType(row.errorType);

  const outcomeText =
    row.correct === null
      ? "Correctness is not available for this row."
      : row.correct
        ? `Outcome check: correct (${displayError}).`
        : `Outcome check: incorrect (${displayError}).`;

  const xaiText =
    hasXaiAllocation(row)
      ? `XAI allocation: foreground ${formatMetric(row.fg, 3, "—")}, background ${formatMetric(row.bg, 3, "—")}.`
      : "XAI allocation is not quantified for this row.";

  return [
    action.note,
    `Case facts: model direction is ${predictedDirection}; true direction is ${actualDirection}; P(up) is ${formatMetric(row.pUp, 3, "not available")}; forward return is ${formatSigned(row.forwardReturn)}.`,
    outcomeText,
    xaiText
  ].join(" ");
}
...
function recommendAction(row) {
  ...
  return {
    ...
    kind: "watch",
    note: "Do not elevate this case as an actionable decision-support candidate. Keep it visible for audit and comparison, but treat it as a non-actionable interior-score or low-trust case."
  };
}
...
function normalizeXai(bucket, fg, bg) {
  ...
  if (Number.isFinite(f) && Number.isFinite(b) && Math.abs(f + b) > 0.001) {
...
function hasXaiAllocation(row) {
  if (!row) return false;
  const fg = number(row.fg);
  const bg = number(row.bg);
  return Number.isFinite(fg) && Number.isFinite(bg) && Math.abs(fg + bg) > 0.001;
}

function prettyErrorType(v) {
  const s = String(v || "").trim();
  if (!s || s === "—" || s === "-") return "—";

  const key = s.toLowerCase().replace(/[\s-]+/g, "_");
  const map = {
    correct_long: "Correct long",
    correct_short: "Correct short",
    false_long: "False long",
    false_short: "False short",
    wrong_long: "False long",
    wrong_short: "False short",
    correct: "Correct",
    incorrect: "Incorrect",
...
function parseTruth(v) {
  ...
  if (["1", "1.0", "true", "yes", "correct"].includes(s)) return true;
  if (["0", "0.0", "false", "no", "incorrect"].includes(s)) return false;
  return null;
}

function inferPredictedClass(pred, pUp, signal) {
  const p = number(pred);
  if (p === 1) return 1;
  if (p === 0) return 0;

  const prob = number(pUp);
...
function inferCorrect(predClass, labelUp) {
  const y = number(labelUp);
  if ((predClass === 1 || predClass === 0) && (y === 1 || y === 0)) {
    return predClass === y;
  }
  return null;
}

function inferErrorType(rawError, predClass, labelUp, correct) {
  ...
  if (predClass === 1 && y === 1) return "correct_long";
  if (predClass === 0 && y === 0) return "correct_short";
  if (predClass === 1 && y === 0) return "false_long";
  if (predClass === 0 && y === 1) return "false_short";

  return correct === null ? "—" : (correct ? "correct" : "incorrect");
}
...
function number(v) {
  ...
  if (typeof v === "string") {
    ...
    if (
      ...
      s === "—" ||
      s === "-" ||
      ["na", "n/a", "nan", "none", "null", "undefined"].includes(s.toLowerCase())
    ) {
      return NaN;
    }
