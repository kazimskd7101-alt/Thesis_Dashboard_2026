...
function buildDecisionNote(row, action) {
  ...
  const outcomeText =
    row.correct === null
      ...
      : row.correct
        ? `Outcome check: correct (${row.errorType || "classified correctly"}).`
        : `Outcome check: incorrect (${row.errorType || "classified incorrectly"}).`;

  const xaiText =
    Number.isFinite(row.fg) || Number.isFinite(row.bg)
